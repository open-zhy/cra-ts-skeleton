...
const reducer = (store: IStorage<IUserPreferences>) =>
  (state: IUserPreferences, action: Action): IUserPreferences => {
...
export const AppSettingsProvider: React.FC<React.PropsWithChildren<AppSettingsProviderProps>> = ({
  ...
  const setValues = (payload: Partial<IUserPreferences>): void =>
    dispatch({
      type: 'SET',
      payload,
    });
